...
const createNewAdminUser = async ({ email, password }) => {
  ...
  try {
    ...
    if (!userPasswordMatches) {
      ...
      await UserModel.findOneAndUpdate(
        { email: user.email, role: systemRoles.USER },
        { $set: updateData },
        { new: true }
      );
    ...
    if (!user.isAdminApproved) {
      throw new APIError(
        httpStatus.BAD_REQUEST,
        "Your account is not approved. Please contact an admin for assistance."
      );
    }
